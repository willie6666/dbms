...
- **觸發時機**：資料表關聯建立時，確保資料完整性 (Referential Integrity)。
- **原生 SQL 語法**：
  ```sql
  ALTER TABLE games 
  ...
  ON DELETE CASCADE;
  ```
- **說明**：這是修改現有資料表結構 (`ALTER TABLE`) 的重要應用。在建立完 `games` 表與 `users` 表後，必須透過 `ALTER TABLE` 補上外鍵約束，強制規定遊戲的 `developer_id` 必須真實存在於 `users` 表中。
